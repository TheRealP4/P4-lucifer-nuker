...
async def help(ctx):
    ...
    embed = discord.Embed(title=" > `LUCIFER!` **Coded By P4**", color= discord.Color(random.randint(0xffffff, 0xffffff)))
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/778450087173029918/783490642441142333/lucifergif1withname.gif")
    ...
    embed.add_field(name=prefix+"**channels**", value="`Channels`\n", inline=False)
    embed.add_field(name=prefix+"**zoom**", value="`Deletes everything`\n", inline=False)
    embed.add_field(name=prefix+"**stream**", value="`Streams`\n", inline=False)
    embed.set_footer(text=f"Request by {ctx.author}", icon_url=ctx.author.avatar_url)
    embed.set_image(url="https://cdn.discordapp.com/attachments/778450087173029918/783499100938960927/lucifergif3.gif")
...
async def channels(ctx):
  ...
  for i in range(100):
    await ctx.guild.create_text_channel(name=f'P4 HOED YOU ')
    print(f"{Fore.RED}Added {channel.name}")
...
async def stream(ctx, *, message): 
    ...
    stream = discord.Streaming(
        name=message,
        url="https://www.twitch.tv/snxfuls", 
    )
...
Ohzey.run('TOKEN_HERE', bot=False)
